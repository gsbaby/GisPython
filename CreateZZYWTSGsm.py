...
LOG_FILE_NAME = "E:\\GisPython\\ZZYW\\logs\\zzywTSGsm"+time.strftime('%Y%m%d',startTime)+".log"
...
def createGisTableZZYWGSM(cellThiesses):
    ...
    print filterSQL
    ZZYWGSMTemp = ArcCatalogPath+"\\"+GisSourcePath+"\\WangYouZZYW.DBO.GIS_THIESS_ZZYW_GSM_TEMP"
    currentDataTableZZYWGSM= ArcCatalogPath+"\\"+DataSourcePath+"\\WANGYOU.GIS_JF_GSM"
    resultZZYWGSM = ArcCatalogPath+"\\"+GisSourcePath+"\\WangYouZZYW.DBO.GIS_THIESS_ZZYW_GSM"+lastMonthEnd.strftime('%Y%m')
...
if __name__ == "__main__":
    try:
        print "create ZZYW  TSGSM"
        logging.info(time.strftime('%Y-%m-%d %H:%M:%S',startTime))
        logging.info("create ZZYM TSGSM table")
        CellThiessSource = "WangYouCellThiess60.sde"
        cellThiessFeatures = ArcCatalogPath+"\\"+CellThiessSource+"\\WangYouCellThiess.DBO.GIS_CELL_TSSPOLY_GSM"+yestday.strftime('%Y%m%d')
