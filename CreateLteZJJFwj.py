...
LOG_FILE_NAME = "E:\\GisPython\\logs\\CreateLteZJJFwj.log"
...
if __name__ == '__main__':
    try:
        # Local variables:
        logging.info("-----------------------start--------------------")
        print "Create LTE ZJJ FWJ Data"
        logging.info("Create LTE ZJJ FWJ Data")
        ...
        InputData = ArcCatalogPath+"\\"+JGYHDbPath+"\\JGYH.DBO.GIS_OBJECT_LTEPNT_ZJJ_FWJ"
        ZJJ_FWJ_DETAIL = ArcCatalogPath+"\\"+JGYHDbPath+"\\JGYH.DBO.GIS_LTE_ZJJ_FWJ_DETAIL"

        rows = arcpy.SearchCursor(InputData)   
        row = rows.next()   
        if row:
            ...
            updateBAngel1 = "  update GIS_LTE_ZJJ_FWJ_DETAIL set B_ANGEL = 450-NEAR_ANGLE   where [NEAR_ANGLE]>90 and [NEAR_ANGLE]<=180;"
            cursor.execute(updateBAngel1)
            updateBAngel2 = "  update GIS_LTE_ZJJ_FWJ_DETAIL set B_ANGEL = 90-NEAR_ANGLE   where [NEAR_ANGLE]>=-180 and [NEAR_ANGLE]<=90;"
            ...
            updateDistanceSql +="from GIS_LTE_ZJJ_FWJ_DETAIL a ,GIS_OBJECT_LTEPNT_ZJJ_FWJ b "
            ...
            updateNearInfoSql = "update a set a.NEAR_CI =b.CI,a.NEAR_SITE_NAME = b.SITE_NAME "
            updateNearInfoSql +=" from GIS_LTE_ZJJ_FWJ_DETAIL a ,GIS_OBJECT_LTEPNT_ZJJ_FWJ b where a.NEAR_FID  = b.OBJECTID"
            cursor.execute(updateNearInfoSql)
            dbConn.commit()
            logging.info("update flag1")
            print "update flag1"
            updateFlag1Sql = "update a set a.FLAG=1 "
            updateFlag1Sql += "from GIS_LTE_ZJJ_FWJ_DETAIL a ,GIS_OBJECT_LTEPNT_ZJJ_FWJ b "
            ...
            updateFlag12Sql +=" from GIS_LTE_ZJJ_FWJ_DETAIL a ,GIS_OBJECT_LTEPNT_ZJJ_FWJ b "
            ...
            updateFlag13Sql += "from GIS_LTE_ZJJ_FWJ_DETAIL a ,GIS_OBJECT_LTEPNT_ZJJ_FWJ b "
            ...
            updateFlag2Sql += "from GIS_LTE_ZJJ_FWJ_DETAIL a ,GIS_OBJECT_LTEPNT_ZJJ_FWJ b "
            ...
            updateFlag22Sql +=" from GIS_LTE_ZJJ_FWJ_DETAIL a ,GIS_OBJECT_LTEPNT_ZJJ_FWJ b "
            ...
            updateFlag23Sql += "from GIS_LTE_ZJJ_FWJ_DETAIL a ,GIS_OBJECT_LTEPNT_ZJJ_FWJ b "
            ...
            updateFlag3Sql += "from GIS_LTE_ZJJ_FWJ_DETAIL a ,GIS_OBJECT_LTEPNT_ZJJ_FWJ b "
            ...
            updateFlag32Sql +=" from GIS_LTE_ZJJ_FWJ_DETAIL a ,GIS_OBJECT_LTEPNT_ZJJ_FWJ b "
            ...
            updateFlag33Sql += "from GIS_LTE_ZJJ_FWJ_DETAIL a ,GIS_OBJECT_LTEPNT_ZJJ_FWJ b "
            ...
            deleteO3Sql = "delete GIS_OBJECT_LTEZJJ_FWJ_O3"
            cursor.execute(deleteO3Sql)
            logging.info("insert o3")
            print "insert o3"
            insertO3Sql = "insert into GIS_OBJECT_LTEZJJ_FWJ_O3   select IN_FID,NEAR_SITE_NAME,DISTANCE,ORDER_ID,FLAG from ( "
            ...
            insertO3Sql += "from GIS_LTE_ZJJ_FWJ_DETAIL a where a.FLAG is not null group by IN_FID,NEAR_SITE_NAME,DISTANCE,FLAG ) t WHERE ORDER_ID <4 "
            cursor.execute(insertO3Sql)
            dbConn.commit()
            print "delete data final detail"
            logging.info("delete data final detail")
            deleteFinalDetail ="delete GIS_OBJECT_LTEZJJ_FINA_DETAIL"
            cursor.execute(deleteFinalDetail)
            print "insert final detail"
            logging.info("insert final detail")
            insertFinalDetail = "insert into GIS_OBJECT_LTEZJJ_FINA_DETAIL select a.IN_FID,a.NEAR_CI,a.NEAR_SITE_NAME,a.DISTANCE,b.ORDER_ID,b.FLAG from GIS_LTE_ZJJ_FWJ_DETAIL a,GIS_OBJECT_LTEZJJ_FWJ_O3 b "
            insertFinalDetail +="where a.IN_FID=b.IN_FID and a.NEAR_SITE_NAME = b.NEAR_SITE_NAME "
            cursor.execute(insertFinalDetail)
            dbConn.commit()
            print "delete GIS_OBJECT_LTEZJJ_FWJ"
            logging.info("delete GIS_OBJECT_LTEZJJ_FWJ")
            deleteFwjZjjSql = "delete GIS_OBJECT_LTEZJJ_FWJ"
            cursor.execute(deleteFwjZjjSql)
            print "insert fwjzjj "
            logging.info("insert fwjzjj")
            insertFwjZjj1 = "insert into  GIS_OBJECT_LTEZJJ_FWJ  select a.IN_FID,avg(DISTANCE) DISTANCE,'',1 from [GIS_OBJECT_LTEZJJ_FINA_DETAIL] a "
            insertFwjZjj1 += "where a.FLAG=1  group by a.IN_FID "
            cursor.execute(insertFwjZjj1)
            insertFwjZjj2 = "insert into  GIS_OBJECT_LTEZJJ_FWJ  select a.IN_FID,avg(DISTANCE) DISTANCE,'',2 from [GIS_OBJECT_LTEZJJ_FINA_DETAIL] a "
            insertFwjZjj2 += "where a.FLAG=2 and a.IN_FID not in (select IN_FID from  GIS_OBJECT_LTEZJJ_FWJ)  group by a.IN_FID "
            cursor.execute(insertFwjZjj2)
            insertFwjZjj3 = "insert into  GIS_OBJECT_LTEZJJ_FWJ  select a.IN_FID,avg(DISTANCE) DISTANCE,'',3 from [GIS_OBJECT_LTEZJJ_FINA_DETAIL] a "
            insertFwjZjj3 += "where a.FLAG=3 and a.IN_FID not in (select IN_FID from  GIS_OBJECT_LTEZJJ_FWJ)  group by a.IN_FID "
            cursor.execute(insertFwjZjj3)
            dbConn.commit()
            print "update fwjzjj ci"
            logging.info("update fwjzjj ci")
            updateFwjZjjSql = "update a set a.CI=b.CI from GIS_OBJECT_LTEZJJ_FWJ a,GIS_OBJECT_LTEPNT_ZJJ_FWJ b     where a.IN_FID = b.OBJECTID "
