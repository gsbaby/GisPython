...
LOG_FILE_NAME = "E:\\GisPython\\logs\\CreateLteZJJThiess.log"
...
if __name__ == '__main__':
    try:
        ...
        InputTable = ArcCatalogPath+"\\"+GISDBPath+"\\WangYouCellThiess.DBO.GIS_CELL_TSSPOLY_LTE"+yestday.strftime('%Y%m%d')
        ZJJ_DETAIL = ArcCatalogPath+"\\"+JGYHDbPath+"\\JGYH.DBO.GIS_CELL_TSSPOLY_LTE_D"+yestday.strftime('%Y%m%d')
        ...
        if row:
            ...
            # Process: 面邻域
            arcpy.PolygonNeighbors_analysis(InputTable, ZJJ_DETAIL, "OBJECTID;CELL_NAME;CI;LATITUDE;LONGITUDE;SITE_NAME", "NO_AREA_OVERLAP", "BOTH_SIDES", "", "METERS", "SQUARE_MILES")
            ...
            updateThiessZjjSql=" update GIS_CELL_TSSPOLY_LTE_D"+yestday.strftime('%Y%m%d')+" set DISTANCE =(dbo.fnGetDistance(src_LATITUDE,src_LONGITUDE,nbr_LATITUDE,nbr_LONGITUDE))*1000"
            cursor.execute(updateThiessZjjSql)
            deleteZjjSql = "truncate table GIS_ZJJ_LTE"
            cursor.execute(deleteZjjSql)
            createZjjThiessSql = " insert into GIS_ZJJ_LTE(CI,ZJJ) select t.src_CI as CI ,avg(DISTANCE) as ZJJ  from GIS_CELL_TSSPOLY_LTE_D"+yestday.strftime('%Y%m%d')+" t"
            createZjjThiessSql += " where t.DISTANCE>0 and t.src_SITE_NAME != t.nbr_SITE_NAME "
            createZjjThiessSql +="group by t.src_CI"
