...
LOG_FILE_NAME = "E:\\GisPython\\ZZYW\\logs\\zzywSXGsm"+time.strftime('%Y%m%d',startTime)+".log"
        ...
        for i in range(dataLen):
            ...
            if(CurrentDataRow["CI"]==str(int(cellThiess["CI"])) and CurrentDataRow["REGION_NAME"]==cellThiess["CITY_NAME"]):
                ...
                for dataField in dataFields:
                    ...
                    if(dataFieldName!="OBJECTID" and dataFieldName!=shapefieldname  and dataFieldName!=cellname and dataFieldName!="SHAPE.STArea()" and dataFieldName!="SHAPE.STLength()" and dataFieldName!=totalincome):
                        resultThiessRow.setValue(dataFieldName,CurrentDataRow[dataFieldName])
                resultThiessRow.TOTAL_INCOME = CurrentDataRow["GSM_ZDYWFB_1060"]+CurrentDataRow["GSM_ZDYWFB_1061"]
...
def dataTable2Array(dataTable,dataFields,filterSQL):
    ...
    while CurrentDataRow:
        ...
        for dataField in dataFields:
            dataFieldName = dataField.name
            if(dataFieldName!="OBJECTID" and dataFieldName!=shapefieldname and dataFieldName!="SHAPE.STArea()" and dataFieldName!="SHAPE.STLength()" and dataFieldName!=cellname and dataFieldName!=totalincome):
...
def createGisTableZZYWGSM(cellThiesses):
    filterSQL = "TIME = TO_DATE('"+lastMonthStart.strftime('%Y-%m-%d 00:00:00')+"','YYYY-MM-DD HH24:MI:SS')"
    print filterSQL
    ZZYWGSMTemp = ArcCatalogPath+"\\"+GisSourcePath+"\\WangYouZZYW.DBO.GIS_SX_ZZYW_GSM_TEMP"
    currentDataTableZZYWGSM= ArcCatalogPath+"\\"+DataSourcePath+"\\WANGYOU.GIS_JF_GSM"
    resultZZYWGSM = ArcCatalogPath+"\\"+GisSourcePath+"\\WangYouZZYW.DBO.GIS_SX_ZZYW_GSM"+lastMonthEnd.strftime('%Y%m')
    if(arcpy.Exists(currentDataTableZZYWGSM)):
        logging.info("do create Gis table")
        createGisTable(resultZZYWGSM,ZZYWGSMTemp,currentDataTableZZYWGSM,cellThiesses,filterSQL)
    else:
...
if __name__ == "__main__":
    try:
        print "create ZZYW  SXGSM"
        logging.info(time.strftime('%Y-%m-%d %H:%M:%S',startTime))
        logging.info("create ZZYM SXGSM table")
        CellThiessSource = "WangYouCellThiess60.sde"
        cellThiessFeatures = ArcCatalogPath+"\\"+CellThiessSource+"\\WangYouCellThiess.DBO.GIS_OBJECT_GSMSX"+yestday.strftime('%Y%m%d')
        ...
        print "||    create ZZYW SXGSM         ||"
        print "======================================"    
        createGisTableZZYWGSM(cellShapes)
    except Exception,e:
        print e
        logging.error(e)
