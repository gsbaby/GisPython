...
LOG_FILE_NAME = "E:\\GisPython\\logs\\CreateGsmZJJThiess.log"
...
if __name__ == '__main__':
    try:
        # Local variables:
        logging.info("-----------------------start--------------------")
        print "Create GSM ZJJ Data"
        logging.info("Create GSM ZJJ Data")
        ...
        InputTable = ArcCatalogPath+"\\"+GISDBPath+"\\WangYouCellThiess.DBO.GIS_CELL_TSSPOLY_GSM"+yestday.strftime('%Y%m%d')
        InputTable1 = ArcCatalogPath+"\\"+GISDBPath+"\\WangYouCellThiess.DBO.GIS_CELL_TSSPOLY_GSM"+yestday.strftime('%Y%m%d')
        InputTable2 = ArcCatalogPath+"\\"+GISDBPath+"\\WANGYOUCELLTHIESS.DBO.GIS_CELL_TSSPOLY_GSM"+yestday.strftime('%Y%m%d')
        ZJJ_DETAIL = ArcCatalogPath+"\\"+JGYHDbPath+"\\JGYH.DBO.GIS_CELL_TSSPOLY_GSM_D"+yestday.strftime('%Y%m%d')
        if(arcpy.Exists(InputTable1)):
            rows = arcpy.SearchCursor(InputTable1)
            InputTable = InputTable1
        else:
            rows = arcpy.SearchCursor(InputTable2)
            InputTable = InputTable2
        row = rows.next()   
        if row:
            ...
            updateThiessZjjSql=" update GIS_CELL_TSSPOLY_GSM_D"+yestday.strftime('%Y%m%d')+" set DISTANCE =(dbo.fnGetDistance(src_LATITUDE,src_LONGITUDE,nbr_LATITUDE,nbr_LONGITUDE))*1000"
            cursor.execute(updateThiessZjjSql)
            deleteZjjSql = "delete GIS_ZJJ_GSM"
            cursor.execute(deleteZjjSql)
            createZjjThiessSql = " insert into GIS_ZJJ_GSM select t.src_CITY_NAME as CITY_NAME,t.src_CI as CI ,avg(DISTANCE) as ZJJ  from GIS_CELL_TSSPOLY_GSM_D"+yestday.strftime('%Y%m%d')+" t"
