...
LOG_FILE_NAME = "E:\\GisPython\\logs\\CreateComplainSheetWeek.log"
logging.basicConfig(filename=LOG_FILE_NAME,level=logging.INFO)
logging.basicConfig(filename=LOG_FILE_NAME,level=logging.ERROR)
lastWeekDateStart = datetime.datetime.now() + datetime.timedelta(days=-(7+datetime.datetime.weekday(datetime.datetime.now())))
...
def dataGis2Table(gisTable,dataTable):
    ...
    while GisDataRow:
        ...
        resultRow.setValue("BEGIN_DATE",lastWeekDateEnd.strftime("%Y%m%d"))
...
if __name__ == '__main__':
    try:
        ...
        print lastWeekDateStart.strftime('%Y-%m-%d 00:00:00')
        ...
        InputFeatures = ArcCatalogPath+"\\"+GISDBPath+"\\WangYouTouSu.DBO.GIS_TOUSU_WEEK"
        POI = ArcCatalogPath+"\\"+GISDBPath+"\\WangYouTouSu.DBO.POI_POINTS_AREA"
        ComplainSheetWeek =ArcCatalogPath+"\\"+GISDBPath+"\\WangYouTouSu.DBO.COMPLAIN_TOUSU_SHEET_WEEK_TMP"
        InsertDataWeek = ArcCatalogPath+"\\"+InsertPath+"\\WANGYOU.F_QL_WT_TOUSU_WEEK"
        arcpy.env.outputCoordinateSystem = arcpy.SpatialReference("WGS 1984")
        print "crarte tmp...begin"
        day = time.strftime('%d',startTime)
        print day
        if(int(day)>=1):
            selectCompSQL ="(TSLB_ID='网络覆盖' or TSLB_ID='手机上网' or TSLB_ID='数据卡（含MIFI）' or TSLB_ID='通话质量') and (TSRQ >= '"+lastWeekDateEnd.strftime('%Y-%m-01 00:00:00')+"' and  TSRQ <= '"+lastWeekDateEnd.strftime('%Y-%m-%d 23:59:59')+"')"
            logging.info(selectCompSQL)
            createComplainRasterFeature(selectCompSQL,InputFeatures,POI,ComplainSheetWeek)
            print "crarte tmp...end"
            print "insert begin..."
            dataGis2Table(ComplainSheetWeek,InsertDataWeek)
            print "insert end..."
    except Exception,e:
        print e
        logging.error(e)
