...
LOG_FILE_NAME = "E:\\GisPython\\logs\\UpdateTDKPI.log"
...
def updateKPI():
    ...
    updateSqlDefault = "UPDATE GIS_CELL_TSSPOLY_TD"+yestday.strftime('%Y%m%d')+" set VOICE_TOTAL_TRAFFIC =0,UL_DL_FLOW=0 "
    ...
    updateSql+=" from GIS_CELL_TSSPOLY_TD"+yestday.strftime('%Y%m%d')+" a,"
    updateSql+="GIS_ZHIBIAO_TD_DAY"+yestday.strftime('%Y%m%d')+" b where a.CI=b.CI and a.CITY_NAME=b.CITY_NAME and a.TIME_STAMP = b.TIME_STAMP"
    print "更新指标值"
    logging.info("更新指标值")
    cursor.execute(updateSql)
    updateMSql = "update a set a.M_VTT = b.VOICE_TOTAL_TRAFFIC,a.M_UDF = b.UL_DL_FLOW"
    updateMSql+= " from GIS_CELL_TSSPOLY_TD"+yestday.strftime('%Y%m%d')+"  a,HF_PM_CELL_DAY_3G_M b "
    ...
    updateWSql+= " from GIS_CELL_TSSPOLY_TD"+yestday.strftime('%Y%m%d')+"  a,HF_PM_CELL_DAY_3G_W b "
    updateWSql+= " where a.CI=b.CI and a.CITY_NAME = b.CITY_NAME;"    
    cursor.execute(updateWSql)
    updateWsType = "UPDATE a  set  a.WS_TYPE = b.WS_TYPE  from GIS_CELL_TSSPOLY_TD"+yestday.strftime('%Y%m%d')+" a,GIS_WS_NAME_TYPE b where a.WS_NAME = b.WS_NAME "
    ...
    updateSXSql+=" from GIS_OBJECT_TDSX"+yestday.strftime('%Y%m%d')+" a,"
    updateSXSql+="GIS_ZHIBIAO_TD_DAY"+yestday.strftime('%Y%m%d')+" b where a.CI=b.CI and a.CITY_NAME=b.CITY_NAME and a.TIME_STAMP = b.TIME_STAMP"
    cursor.execute(updateSXSql)
    updateSXMSql = "update a set a.M_VTT = b.VOICE_TOTAL_TRAFFIC,a.M_UDF = b.UL_DL_FLOW"
    updateSXMSql+= " from GIS_OBJECT_TDSX"+yestday.strftime('%Y%m%d')+"  a,HF_PM_CELL_DAY_3G_M b "
    ...
    updateSXWSql+= " from GIS_OBJECT_TDSX"+yestday.strftime('%Y%m%d')+"  a,HF_PM_CELL_DAY_3G_W b "
    updateSXWSql+= " where a.CI=b.CI and a.CITY_NAME = b.CITY_NAME;"    
    cursor.execute(updateSXWSql)
    updateSXWsType = "UPDATE a  set  a.WS_TYPE = b.WS_TYPE  from GIS_OBJECT_TDSX"+yestday.strftime('%Y%m%d')+" a,GIS_WS_NAME_TYPE b where a.WS_NAME = b.WS_NAME "
    cursor.execute(updateSXWsType)
    dbConn.commit()
    dbConn.close()

def createYun():
    StopOrStartService(serviceFolder,"DynamicTDRasterHW","stop")
    StopOrStartService(serviceFolder,"DynamicTDRasterLL","stop")
    ...
    StopOrStartService(serviceFolder,"DynamicTDRasterHW","start")
    StopOrStartService(serviceFolder,"DynamicTDRasterLL","start")
...
if __name__ == '__main__':
    try:
        ...
        CellThiessYunHW = ArcCatalogPath+"\\"+SITEDBPath+"\\WangYouSite.dbo.GIS_RASTER_TSS_TD_HW"
        CellThiessYunLL = ArcCatalogPath+"\\"+SITEDBPath+"\\WangYouSite.dbo.GIS_RASTER_TSS_TD_LL"
        ...
        zhiBiaoTableMOrg = ArcCatalogPath+"\\"+DataSourcePath+"\\WANGYOU.HF_PM_CELL_DAY_3G_M"
        zhiBiaoTableWOrg = ArcCatalogPath+"\\"+DataSourcePath+"\\WANGYOU.HF_PM_CELL_DAY_3G_W"
        zhiBiaoTableGisM = ArcCatalogPath+"\\"+GISDBPath+"\\WangYouCellThiess.dbo.HF_PM_CELL_DAY_3G_M"
        zhiBiaoTableGisW = ArcCatalogPath+"\\"+GISDBPath+"\\WangYouCellThiess.dbo.HF_PM_CELL_DAY_3G_W"
        ...
        createYun()
        arcpy.AddIndex_management(CellThiessFinal, "WS_TYPE", "tdwsTypeIndex", "NON_UNIQUE", "NON_ASCENDING")
        arcpy.AddIndex_management(CellShanXing, "WS_TYPE", "tdSXWSTypeIndex", "NON_UNIQUE", "NON_ASCENDING")
