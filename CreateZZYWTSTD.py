...
LOG_FILE_NAME = "E:\\GisPython\\ZZYW\\logs\\zzywTSTD"+time.strftime('%Y%m%d',startTime)+".log"
...
def createGisTableZZYWTD(cellThiesses):
    ...
    print filterSQL
    ZZYWTDTemp = ArcCatalogPath+"\\"+GisSourcePath+"\\WangYouZZYW.DBO.GIS_THIESS_ZZYW_TD_TEMP"
    currentDataTableZZYWTD= ArcCatalogPath+"\\"+DataSourcePath+"\\WANGYOU.GIS_JF_TD"
    resultZZYWTD = ArcCatalogPath+"\\"+GisSourcePath+"\\WangYouZZYW.DBO.GIS_THIESS_ZZYW_TD"+lastMonthEnd.strftime('%Y%m')
...
if __name__ == "__main__":
    try:
        print "create ZZYW  TSTD"
        logging.info(time.strftime('%Y-%m-%d %H:%M:%S',startTime))
        logging.info("create ZZYM TSTD table")
        CellThiessSource = "WangYouCellThiess60.sde"
        cellThiessFeatures = ArcCatalogPath+"\\"+CellThiessSource+"\\WangYouCellThiess.DBO.GIS_CELL_TSSPOLY_TD"+yestday.strftime('%Y%m%d')
