...
LOG_FILE_NAME = "E:\\GisPython\\logs\\GisTouSuWeek.log"
logging.basicConfig(filename=LOG_FILE_NAME,level=logging.INFO)
logging.basicConfig(filename=LOG_FILE_NAME,level=logging.ERROR)
lastWeekDateEnd = datetime.datetime.now() + datetime.timedelta(days=-(1+datetime.datetime.weekday(datetime.datetime.now())))
spatialReference = arcpy.SpatialReference(4326)
...
if __name__ == '__main__':
    ...
    logging.info(lastWeekDateEnd.strftime('%Y-%m-%d 00:00:00'))
    print lastWeekDateEnd.strftime('%Y-%m-%d 00:00:00')
    ...
    ComplainSheetPoint = ArcCatalogPath+"\\"+GISDBPath+"\\WangYouTouSu.DBO.GIS_TOUSU_WEEK"
    
    createGisTable(ComplainSheetPoint,GisTouSuWeekTemp)
    
    complainSheetCursor=arcpy.SearchCursor(infc,"TIME>=TO_DATE('"+lastWeekDateEnd.strftime('%Y-%m-01 00:00:00')+"','YYYY-MM-DD HH24:MI:SS') and TIME<=TO_DATE('"+lastWeekDateEnd.strftime('%Y-%m-%d 23:59:59')+"','YYYY-MM-DD HH24:MI:SS')")
